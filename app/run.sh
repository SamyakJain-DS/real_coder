...
run_all_tests() {
  ...
  COMPILE_DIR="/tmp/resolver_test_classes"
  ...
  # Compile implementation sources if present (failures tolerated)
  ...
  # Symlink /app/data into the current directory so tests can resolve data/registry.json
  if [ -d /app/data ] && [ ! -e data ]; then
    ln -sf /app/data data
  fi
